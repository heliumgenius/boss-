...
# message_template supports {message} placeholder (formatted in _handle_response)
ERROR_CODE_MAP: dict[int, tuple[type[BossApiError], str]] = {
    37: (SessionExpiredError, ""),  # SessionExpiredError has its own message
    17: (ParamError, "{message}"),
    19: (ParamError, "{message}"),
    9: (RateLimitError, ""),  # RateLimitError has its own message
}
